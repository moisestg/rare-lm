...
class PointerSentinelMixtureLM(object):
	...
	def __init__(self, config, is_training, pretrained_emb=None):
		...
		# switch_y contains a 1 if the word in that position is a name and 0 if not
		self.switch_y = switch_y = tf.placeholder(dtype=tf.float32, shape=[batch_size*num_steps], name="switch_y") # float32 because of sigmoid_loss
		...
		with tf.name_scope("embedding_layer"):
			with tf.device("/cpu:0"): # embedding operation not supported in GPU!
		...
		with tf.name_scope("softmax_layer"): 
			W_softmax = tf.get_variable("W_softmax", dtype=tf.float32, shape=[final_hidden_size, vocab_size])
			...
			# Usually we would use 'tf.nn.softmax_cross_entropy_with_logits()' but as we need access to the probabilites
		...
		with tf.name_scope("pointer_layer"):
			initial_attention_states = self.initial_attention_states = tf.get_variable("initial_attention_states", dtype=tf.float32, shape=[batch_size, final_hidden_size, attention_length], trainable=False)
			...
			for step in range(num_steps):
				...
				initial_attention_states = tf.concat([initial_attention_states[:,:,1:], tf.expand_dims(current_hidden_states, axis=-1)], axis=-1) # Update cache of attention_states with current_hidden_states
			...
			# Expose the cache content at the end of an iteration so the next minibatch can be initialized properly
